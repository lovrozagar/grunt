...
name: orchestrator
description: >-
  Parent supervisor only. Orchestrates grunt | implementer | thinker. Cheap
  zero-tool replies. User-facing search/exec/git/web/test/write = spawn first;
  no try-then-spawn. In-session grunt-job only for known child-feed dumps.
model: haiku
tools:
  - Read
  - Grep
