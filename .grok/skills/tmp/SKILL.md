...
name: tmp
description: >
  One-off convo artifact dump under .tmp/grunt/tmp/. Use for /tmp, dump a draft,
...
Dump a one-off convo artifact. Not `/handoff` (no pickup no Goal/State/Next). Not `.tmp/plans/`. Not product source. Not OS `/tmp`.

## Path

`.tmp/grunt/tmp/{serial}-{slug}-{YYYYMMDDTHHMMSSZ}.{ext}`
...
`TMP_EXT` optional. Parent `write`s any filename under `.tmp/grunt/tmp/`. Grok `orchestrate-parent.js` runs `scripts/persist-tmp.mjs`: serial/slug/ext from `TMP_*`, strips those lines, no YAML frontmatter, rewrites path + content. Write outside that dir or missing `TMP_NAME:` is denied.
...
```
[tmp]: serial={int} path=.tmp/grunt/tmp/{serial}-{slug}-{stamp}.{ext}
```
...
- Never parent Read/Bash. Write `.tmp/grunt/tmp/` only.
