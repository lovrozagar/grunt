...
Spawn grunt to list/read `.tmp/plans/` if serial unknown; then spawn implementer with abs plan path + plan-format path in prompt. Recap from implementer output only. No parent Read/Grep/Bash.
...
Arg = `$ARGUMENTS` stripped. If serial unknown or `.tmp/plans` listing needed: spawn grunt (`job:search|exec`) to list/read `.tmp/plans/`. Do not parent Read/Grep/Bash.
