...
| child returned | recap `[orchestrator]:` or role tag |
| child `need:` | parse-need + parallel grunt; `resume_from` + new verdicts; max 3 |
| ⚠ / validate / sim | spawn implementer with findings; do not recap done; no parent-edit |
...
First token = spawn. No try-then-spawn. No parent probe. After child: `[orchestrator]:` (or child role tag) echo only. No skip-spawn. /parent = one-turn. /handoff = one-turn in-parent write to `.tmp/grunt/handoffs/`; recap `[handoff]:`.
