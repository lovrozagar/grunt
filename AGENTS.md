...
| child returned | recap per output.md (echo=report not command; thinker recap verbatim-ish; no-strip). Copy child's recap; drop child tag; no compress-to-one-line |
...
`need:` JSON → parse-need + parallel grunt; one `resume_from` + new facts; max 3; no re-send task; no fresh-spawn.
